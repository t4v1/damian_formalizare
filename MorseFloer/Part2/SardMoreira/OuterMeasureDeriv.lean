...
import Mathlib
import MorseFloer.Part2.SardMoreira.UnifDoublingCover
...
instance instClosedBallCoveringMeasureOfIsUnifLocDoublingMeasure [BorelSpace α] [SecondCountableTopology α]
  ...
  exists_closedBall_covering_tsum_measure_le := by
    ...
    set t : Set (α × ℝ) :=
      {(c, r) : α × ℝ | c ∈ s ∧ r ∈ f c ∩ Ioo 0 (IsUnifLocDoublingMeasure.scalingScaleOf μ 3) ∧ closedBall c r ⊆ U}
    obtain ⟨u, hus, huc, hud, hμ⟩ : ∃ u ⊆ t, u.Countable ∧
        u.PairwiseDisjoint (fun a ↦ closedBall a.1 a.2) ∧
        μ (s \ ⋃ a ∈ u, closedBall a.1 a.2) = 0 := by
      apply Vitali.exists_disjoint_covering_ae μ s t (IsUnifLocDoublingMeasure.scalingConstantOf μ 3) Prod.snd Prod.fst
      · exact fun _ _ ↦ Subset.rfl
      · exact fun x hx ↦ IsUnifLocDoublingMeasure.measure_mul_le_scalingConstantOf_mul _ (by simp) hx.2.1.2.2.le
      · intro x hx
      ...
      · intro x hx ε hε
        ...
        rcases hf x hx (ε ⊓ δ ⊓ IsUnifLocDoublingMeasure.scalingScaleOf μ 3)
          (lt_min (lt_min hε hδ₀) (IsUnifLocDoublingMeasure.scalingScaleOf_pos _ _)) with ⟨r, hrf, hr₀, hrε⟩
        rw [lt_min_iff, lt_min_iff] at hrε
        refine ⟨(x, r), ⟨hx, ⟨hrf, hr₀, hrε.2⟩, ?_⟩, hrε.1.1.le, rfl⟩
        grw [hrε.1.2, hδU]
    rcases IsUnifLocDoublingMeasure.exists_closedBall_covering_tsum_measure_le_of_measure_zero μ (ENNReal.half_pos hε₀).ne'
    ...
    · rw [tsum_congr_set_coe (fun x ↦ μ (closedBall x (goodR x))) Set.union_sdiff_self.symm]
      ...
      grw [← measure_biUnion huc hud (fun _ _ ↦ measurableSet_closedBall),
        ENNReal.tsum_mono_subtype (fun x ↦ μ (closedBall x (r' x))) Set.sdiff_subset, ← ε.add_halves,
...
lemma outerMeasure_null_of_forall_le_mul_ae_null {μ : Measure α} [SigmaFinite μ]
  ...
  · calc
      ν (s ∩ {x | C x = 0}) ≤ 0 * μ (s ∩ {x | C x = 0}) := by
        ...
        grw [inter_subset_left (t := {x | C x = 0})]
  ...
  · set t := s \ {x | C x = 0}
    ...
    calc
      ...
      _ ≤ ∑' n : ℕ, n * μ {x ∈ t | C x ≤ n} := by
        ...
        · intro x hx
          simp only [t]
          grw [sep_subset, Set.sdiff_subset, ← (mod_cast hx.2 : (C x : ℝ≥0∞) ≤ n)]
          exact h x hx.1.1
