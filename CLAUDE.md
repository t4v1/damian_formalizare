...
| `Part2/SymplecticComponents.lean` | Prop 7.1.4 | 0 | **`Sp(2n)±` are path-connected**, without Lemma 7.1.5: Cayley transform `A ↦ (A+1)(A−1)⁻¹` onto the Hamiltonian matrices without the eigenvalue `1`; the segment of `HamiltonianSpectrum`; an adapted symplectic basis (a basis of `ker(N−2)`, its `ω`-dual basis of `ker(N+2)`, a symplectic basis of `ker N` from Proposition 5.1.1) conjugating `N` to `HN(B) = [[B, 0], [0, −Bᵀ]]`, `B` diagonal with entries `2` on the first `k` coordinates; merging two entries `2` by `2(1−s)R(πs/2)` in two coordinates; `det(cay N − 1)` has the sign of `(−3)^k`, and a path in `Sp(2n)⋆` keeps the sign of `det(A − 1)` (connectedness of the range) |
| `Part2/Ch7.lean` | 7 Maslov, Conley–Zehnder | 2 | index axiomatised; dimension two in full; Lemma 7.2.3, Remark 7.1.2, `Δ` well defined, `exp(θJ₂) = rot θ` proved; **Theorem 7.1.3 proved**: `exists_isRho` exhibits `Rho.rho`, which satisfies all eight clauses; **Lemma 7.1.6 proved** for it (the lifts on `Sp(2n)±`); **Lemma 7.2.4 proved** from the axioms of 7.2.1 alone, for `n ≥ 2` (false for `n = 1`), by the index recurrence `μ(ℓ+2) + μ(ℓ−2) = 2μ(ℓ)` of the rotation paths; **Proposition 7.1.4 proved** (both halves) without Lemma 7.1.5, from `SymplecticComponents.lean` |
