...
3. **Schauder theory for `∂̄`, which is what blocks Chapter 6.** All six remaining
   ...
   **Schauder estimate**, at order zero (`holder_fderiv_cauchyTransform`) and then at every
   ...
   `exists_isHolderC_comp₂` applies with both inner functions compactly supported. **Two gaps
   remain before 6.5.3 itself**: the target is `ℝ^{2n}`, not `ℂ` — the cheap route is *not* to
   redo the Cauchy transform for `ℂⁿ`-valued maps but to generalise the chain rule to an outer
   function of several variables and run the bootstrap on all components at once, since the
   components are coupled only through the right-hand side; and Chapter 6 states the equation
   for `u : ℝ → ℝ → ℝ^{2n}` with `∂u/∂s + J₀ ∂u/∂t + ∇H_t(u) = 0`, which must be matched with
   the `∂̄` of a map on `ℂ`. With 6.5.3 in hand, 6.5.6 and 6.5.7 still need Ascoli, and 6.5.4 and
