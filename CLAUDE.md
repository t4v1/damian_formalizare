...
| `Part2/Rho.lean` | **§7.3.b–c: the map ρ, defined and continuous** | 0 | `rho n A = ∏_{μ ∈ Spec} factor μ` with `factor = μ^{σ(μ)}` on the upper unit circle, `(−1)^{m(−1)/2}` at `−1`, `(−1)^{m(μ)}` on `(−1, 0)`, `1` elsewhere, and `σ(μ) = 2 m₊(μ) − m(μ)` through `HermitianIndex.posIndex` on `E_μ` (the book's Prop 7.3.5 form, with `(−1)^{m₀/2}` split along the pairs `μ, 1/μ`); **`continuousOn_rho`** (Theorem 7.1.3, continuity clause): at `A₀`, disjoint discs about its distinct eigenvalues capture all eigenvalues of nearby `A` (roots continuity), `ρ` is the product of the local products over the discs, and each local product tends to the factor of the centre — trivially off the circle and `[−1, 0]`; by conjugate pairing on `(−1, 0)`; on the circle by `∑ σ(μ) = σ(λ)` (Cor 7.3.9: the disc's eigenspace sum has signature `∑ σ` over the circle eigenvalues, the off-circle ones pairing into hyperbolic planes of signature `0`, and that signature is locally constant by `SignatureContinuity`) plus an estimate `‖∏ μ^{e} − λ^{∑ e}‖ ≤ N² ε` for unit `μ` within `ε` of `λ`; at `−1` by the parity identity of Prop 7.3.10, counting the eigenvalues of the disc along their orbits under conjugation and inversion, with `m(−1)` even. The remaining clauses of `IsRho` (block sums, the unitary determinant) are not yet proved for this `ρ`; norm one and the inverse are in `RhoProperties.lean`, naturality and the transpose in `RhoNaturality.lean`, the normalisation in `RhoNormalisation.lean` |
