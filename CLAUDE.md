...
3. **Schauder theory for `∂̄`, which is what blocks Chapter 6.** All six remaining
   ...
   **Schauder estimate**, at order zero (`holder_fderiv_cauchyTransform`) and then at every
   ...
   of `∂̄u = F ∘ u` with `F` smooth is smooth, which is the analytic core of 6.5.3. **Three
   gaps remain before 6.5.3 itself**: the target is `ℝ^{2n}`, not `ℂ`, so the whole
   Cauchy-transform development has to be read for `ℂⁿ`-valued maps (the kernel acts
   componentwise, but the files are written for `ℂ → ℂ`); the Hamiltonian depends on `t`, so
   the nonlinearity is `F z (u z)` rather than `F (u z)`; and Chapter 6 states the equation for
   `u : ℝ → ℝ → ℝ^{2n}` with `∂u/∂s + J₀ ∂u/∂t + ∇H_t(u) = 0`, which must be matched with the
   `∂̄` of a map on `ℂ`. With 6.5.3 in hand, 6.5.6 and 6.5.7 still need Ascoli, and 6.5.4 and
