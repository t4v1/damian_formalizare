...
| `Part2/Ch7.lean` | 7 Maslov, Conley–Zehnder | 1 | index axiomatised; dimension two in full; Lemma 7.2.3, Remark 7.1.2, `Δ` well defined, `exp(θJ₂) = rot θ` proved; **Theorem 7.1.3 proved**: `exists_isRho` exhibits `Rho.rho`, which satisfies all eight clauses; **Lemma 7.1.6 proved** for it (the lifts on `Sp(2n)±`); **Lemma 7.2.4 proved** from the axioms of 7.2.1 alone, for `n ≥ 2` (false for `n = 1`), by the index recurrence `μ(ℓ+2) + μ(ℓ−2) = 2μ(ℓ)` of the rotation paths; **Proposition 7.1.4 proved** (both halves) without Lemma 7.1.5, from `SymplecticComponents.lean`; **Lemma 7.1.5 proved** from the same normal form; only 7.2.1 is assumed, blocked by `π₁(Sp(2n)) ≅ ℤ` (simple connectivity of `SU(n)`) for its converse homotopy clause |
