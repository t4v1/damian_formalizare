...
* `MeasureTheory.setLIntegral_ball_eq_lintegral_toSphere_lintegral_Ioo`: the same, localised
  to a ball.
...
theorem setLIntegral_ball_rpow_neg [Nontrivial E] (x : E) {r a : ℝ} (hr : 0 < r)
  ...
  have key : ENNReal.ofReal (r ^ ((finrank ℝ E : ℝ) - a) / ((finrank ℝ E : ℝ) - a)) *
    ...
    congr 1
    field_simp
  rw [rieszKernelConst_def, ENNReal.coe_mul, ENNReal.coe_mul, hb,
...
The compact-support hypothesis is not needed for this estimate; it is kept for uniformity
with `MeasureTheory.eLpNorm_le_eLpNorm_fderiv` and with the essential-supremum form below. -/
theorem enorm_sub_le_morreyConst_mul_rpow_mul_eLpNorm_fderiv [Nontrivial E]
    {u : E → F} (hu : ContDiff ℝ 1 u) (_h2u : HasCompactSupport u)
